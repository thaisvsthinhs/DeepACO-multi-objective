...
class ACO():
    ...
    def gen_path_costs(self, paths):
        ...
        ttime = self.travel_time[u, v]
        return torch.sum(self.w_d * dist + self.w_t * ttime, dim=1)
...
if __name__ == '__main__':
    ...
    dist = torch.norm(coords[:, None] - coords, dim=2)
    dist[torch.arange(len(dist)), torch.arange(len(dist))] = 1e10
    tm = dist / 2.0  # giả sử speed=2.0
    aco = ACO(distances=dist, travel_time=tm, w_d=0.7, w_t=0.3)
